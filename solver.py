...
class Solver():
    def __init__(self, args):
        ...
        if args.strategy == "tree": 
            self.model = Tree_transfomer(  vocab_size= self.vocab_size, N = modelConfig['N_layer'] , No_consti =  modelConfig['Consti'] , d_model= modelConfig['d_model'], 
    ...
    def evaluate(self):
        ...
        with torch.no_grad():
            ...
            for step, batch in enumerate(self.data_util.val_loader):
                ...
                ground_truth_indices = np.argmax(labels, axis=-1)

            

            
                sentiment = ['Clean', "Offensive", "Hate"]

        
            
                predictions_Sentiment = [sentiment[idx]  for idx in predictions_indices]
                ground_truths_Sentiment = [sentiment[idx]  for idx in ground_truth_indices ]
    ...
    def test(self):
        ...
        with torch.no_grad():
            epoch_progress = tqdm(total=len(self.data_util.test_loader), position=0)
        ...
        return  sentiment_precision,  sentiment_recall,  Sentiment_f1
    ...
    def train(self):
        ...
        sentiment_precision,  sentiment_recall, sentiment_f1  = self.test()
