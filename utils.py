...
def process_data(df): 
    ...
    for _ , row in df.iterrows():
        ...
        data_dict["comment"]= row['free_text']
       

        data_dict["label"]= row['label_id']
...
class data_utils():
    def __init__(self, args):
        ...
        df_train["free_text"] = df_train["free_text"].astype(str).fillna("")
  
        df_val = pd.read_csv(args.valid_path,  encoding = 'utf8')
        df_val["free_text"] = df_val["free_text"].astype(str).fillna("")
        ...
        if os.path.exists(os.path.join(args.model_dir,"vocab.json" )) and os.path.exists(os.path.join(args.model_dir,"merges.txt" )): 
            ...
            tokenizer.train_from_iterator(df_train["free_text"], vocab_size=34000, min_frequency=2,
        ...
        if args.test : 
            ...
            df_test["free_text"] = df_test["free_text"].astype(str).fillna("")
